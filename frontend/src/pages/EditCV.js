...
const EditCV = () => {
  ...
  const [address, setAddress] = useState(cvs[index].address)
  const [phone, setPhone] = useState(cvs[index].phone)
  const [email, setEmail] = useState(cvs[index].email)

  const [title, setTitle] = useState(cvs[index].title)
  const [name, setName] = useState(cvs[index].name)
  const [profile, setProfile] = useState(cvs[index].profile)

  const [website, setWebsite] = useState(cvs[index].website)
  const [github, setGithub] = useState(cvs[index].github)
  const [linkedin, setLinkedin] = useState(cvs[index].linkedin)

  const [experience, setExperience] = useState(cvs[index].experience)
  ...
  const [skills, setSkills] = useState(cvs[index].skills)
    const [skill, setSkill] = useState('')

  const [references, setReferences] = useState(cvs[index].references)
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch('https://cv-creator.onrender.com:4005' + '/api/cvs/' + cvs[index]._id, {
      method: 'PATCH',
          ...
          <div className="school-entry">
            <div className="school-container">
              ...
              <div className="sch-duration-container">
                <input className="date-input" type="date" value={schdatefrom} onChange={(e) => setSchDatefrom(e.target.value)} />
                <input className="date-input" type="date" value={schdateto} onChange={(e) => setSchDateto(e.target.value)} />
              </div>
            </div>
